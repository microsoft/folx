...
from folx import forward_laplacian, wrap_forward_laplacian, deregister_function, register_function
        ...
        for f in functions:
            for sparsity in [0, x.size]:
                with self.subTest(sparsity=sparsity, f=f):
                    ...
                    self.assertTrue(np.allclose(y.jacobian.dense_array, jacobian(f, x).T), msg=f"{f}")
                    self.assertTrue(np.allclose(y.laplacian,  laplacian(f, x)), msg=f"{f}")
