...
def batched_vmap(
    ...
    def result(*args, **kwargs):
        ...
        in_axes_flat, in_tree = jtu.tree_flatten(in_axes, lambda x: isinstance(x, int) or x is None)
        ...
        def merge(mapped_args):
            ...
            return tuple(
                next(x_iter) if m else next(y_iter)
                for m in mask
            )
        
        batch_size = jtu.tree_leaves(mapped_args[0])[0].shape[mapped_axes[0]]
        assert (batch_size == x.shape[ax] for arg, ax in zip(mapped_args, mapped_axes) for x in jtu.tree_leaves(arg))
